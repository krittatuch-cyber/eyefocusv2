// app/(user)/user/transfer-stocks/page.tsx
"use client";

import React, { useEffect, useState, useCallback } from "react";
import { useI18n } from "@/lib/i18n-context";
import {
  ArrowRightLeft, Search, Plus, Minus, Loader2, AlertCircle,
  CheckCircle2, ChevronDown, ChevronUp, Package, Building2,
} from "lucide-react";
...
export default function TransferStocksPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setBranches(bList);
      if (bList.length > 0) setFromBranchId(bList[0].id);
      if (bList.length > 1) setToBranchId(bList[1].id);
    } catch (err: any) {
  ...
  const handleSubmit = async () => {
    if (!fromBranchId || !toBranchId || cart.length === 0) return;
    if (fromBranchId === toBranchId) {
      setError(locale === "th" ? "ต้นทางและปลายทางต้องไม่ใช่สาขาเดียวกัน" : "Source and destination must be different branches");
      return;
    }
    ...
    <div className="space-y-4 pb-32">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold uppercase tracking-wider text-slate-800 dark:text-white">
          {locale === "th" ? "โอนสต็อกสินค้า" : "Transfer Stock"}
        </h2>
      </div>

      {/* Success */}
      {success && (
        <div className="flex items-center gap-2 px-4 py-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs text-emerald-700">
          <CheckCircle2 className="w-4 h-4 shrink-0" />
          <span className="font-semibold">{locale === "th" ? "โอนสต็อกสำเร็จ!" : "Stock transferred successfully!"}</span>
        </div>
      )}
        ...
        <>
            ...
            <div className="grid grid-cols-1 gap-3">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 flex items-center gap-1">
                  <Building2 className="w-3 h-3" />{locale === "th" ? "สาขาต้นทาง (ตัดสต็อก)" : "From Branch (deduct)"}
                </label>
                <select
                  value={fromBranchId}
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1.5 flex items-center gap-1">
                  <Building2 className="w-3 h-3" />{locale === "th" ? "สาขาปลายทาง (รับสต็อก)" : "To Branch (receive)"}
                </label>
                <select
                  value={toBranchId}
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={locale === "th" ? "ค้นหาสินค้าเพื่อโอน..." : "Search product to transfer..."}
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-64 overflow-y-auto">
                  {filtered.slice(0, 20).map(p => (
                    <div key={p.id} className="flex items-center justify-between px-4 py-3">
                      ...
                      <button
                        onClick={() => addToCart(p)}
                        className="shrink-0 ml-3 w-8 h-8 rounded-lg bg-accent text-white flex items-center justify-center hover:bg-accent-hover"
                      >
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-sm text-slate-900 dark:text-white">
                    {locale === "th" ? "รายการโอน" : "Transfer List"}
                  </span>
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800 border-t border-slate-100 dark:border-slate-700">
                  {cart.map(item => (
                    <div key={item.product.id} className="flex items-center justify-between px-4 py-3">
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-xs truncate">{item.product.name}</p>
          ...
          <div className="bg-white dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700/60 rounded-2xl p-4">
            ...
            <textarea
              ...
              placeholder={locale === "th" ? "เหตุผลการโอน เช่น สต็อกสาขาหลักเกิน..." : "Transfer reason, e.g., excess stock..."}
              rows={2}
          ...
          <button
            ...
            disabled={cart.length === 0 || !fromBranchId || !toBranchId || submitting}
          ...
          >
            ...
            <span>{locale === "th" ? `โอนสต็อก (${totalItems} ชิ้น)` : `Transfer Stock (${totalItems} pcs)`}</span>
          </button>
        </>
      )}
    </div>
